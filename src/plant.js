...
class Plant{
  constructor(influent, configuration, parameters){
    ...
    */
    ...
    //check input objects
    if(!this.constants) throw new Error('Plant kinetic constants not defined');
  ...
  run(debug_mode){
    ...
    //after the primary settler, the object "pst" contains the state variables to keep track from now on

    //execute modules (as, nit, dn, bpr)
    let as  = null;
    let nit = null;
    let dn  = null;
    let bpr = null;
    ...
    //execute capacity estimation module AFTER BPR (TODO)
    {
      ...
      if(Q   > 1.05*cap.Q_ADWF.value) throw new Error(`Q > Q_ADWF: plant overloaded (more than 5%)`);
      if(X_T > 1.05*cap.X_Tave.value) throw new Error(`X_T > X_Tave: plant overloaded (more than 5%)`);
    }
